...
- plan (core skill for creating comprehensive implementation plans)
- research (for gathering technical requirements and best practices)
- sequential-thinking (for breaking down complex features into actionable phases)
...
- **IMPORTANT:** Respect the rules in `./.claude/rules/development-rules.md` and `./docs/code-standards.md` and `./.claude/rules/coding-convention.md`.
...
- Active Plan: .claude/tasks/FEAT-001/plan
- Reports Path: .claude/tasks/FEAT-001/reports/
...
|--------------------------|---------------------------|
| `Plan dir: .claude/tasks/{task-id}/plan/` | `.claude/tasks/FEAT-001/plan/` |
...
Example:
```bash
node .claude/scripts/set-active-task.cjs GH-88-add-authentication
```
