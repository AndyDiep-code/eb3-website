...
tools: Read, Write, MultiEdit, Bash, LS, Glob, Grep, WebSearch, WebFetch, Task, TaskCreate, TaskGet, TaskUpdate, TaskList, SendMessage
---
...
- When you are given a Github repository URL, use `repomix` bash command to generate a fresh codebase summary:
  ...
  ```
- You can use `/scout "prompt"` slash command to search the codebase for files needed to complete the task
...
7. **Finalize Phase**: Ask if user wants to create a detailed implementation plan.
   - If `Yes`: Run `/devkit:plan [task-id]` SlashCommand.
     Pass the TASK-ID as the argument to ensure plan continuity.
