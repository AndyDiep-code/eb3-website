...
# Agent Teams - Devkit-Native Orchestration Engine
...
```
/devkit:agent-team <template> <context> [flags]
```

**Templates:** `devkit:research`, `cook`, `devkit:code-review`, `devkit:debug`
...
4. Do NOT fall back to subagents. `/devkit:agent-team` MUST use Agent Teams or abort.
...
Agent(
  ...
  prompt: "full instructions + Devkit Context Block",
...
## Devkit Context Block

Every teammate spawn prompt MUST include this context at the end:

```
Devkit Context:
- Work dir: {DEVKIT_PROJECT_ROOT or CWD}
- Reports: {DEVKIT_REPORTS_PATH or "./.claude/tasks/[task-id]/reports/" or "./.claude/tasks/reports/" (if task-id is not provided)}
- Plans: {DEVKIT_PLANS_PATH or "./.claude/tasks/[task-id]/plan/" or "./.claude/tasks/plan/" (if task-id is not provided)}
- Branch: {DEVKIT_GIT_BRANCH or current branch}
- Naming: {DEVKIT_NAME_PATTERN or "YYMMDD-HHMM"}
- Active plan: {DEVKIT_ACTIVE_PLAN or "none"}
...
## ON `/devkit:agent-team research <topic>` [--researchers N]:

*Wraps /devkit:research skill -- scope, gather, analyze, report.*
...
3. **CALL** `TaskCreate` x N -- one per angle:
   ...
   - Description: `Investigate <angle> for topic: <topic>. Save report to: {DEVKIT_REPORTS_PATH}/researcher-{N}-{DEVKIT_NAME_PATTERN}-{topic-slug}.md. Format: Executive summary, key findings, evidence, recommendations. Mark task completed when done. Send findings summary to lead.`
...
4. **SPAWN** teammates x N via `Agent` tool:
   ...
   - `name: "researcher-{N}"`
   - Prompt: task description + Devkit Context Block
...
6. **READ** all researcher reports from `{DEVKIT_REPORTS_PATH}/`

7. **SYNTHESIZE** into: `{DEVKIT_REPORTS_PATH}/research-summary-{DEVKIT_NAME_PATTERN}-{topic-slug}.md`
...
## ON `/devkit:agent-team cook <plan-path-or-description>` [--devs N]:

*Wraps /devkit:cook skill -- plan, code, test, review, finalize.*
...
4. **SPAWN** developer teammates x N via `Agent` tool:
   ...
   - `name: "dev-{N}"`
   - Prompt: task description + plan context + Devkit Context Block
...
6. **MERGE** worktree branches (if `isolation: "worktree"` was used):
   ...
   - Report branches to user: "Worktree branches ready to merge: `<branch-a>`, `<branch-b>`. Please merge them manually."
   - If conflicts: instruct user to resolve conflicts, then `git add . && git merge --continue`
   - Cleanup: `git worktree remove <path>` for each worktree (read-only — removes the directory, not a write to history)
   - Verify: `git log --oneline --graph` to confirm merge topology

7. **DOCS SYNC EVAL** (MANDATORY for cook -- from /devkit:cook finalize):
   ```
...
## ON `/devkit:agent-team review <scope>` [--reviewers N]:

*Wraps /devkit:code-review skill -- scout, review, synthesize with evidence gates.*
...
3. **CALL** `TaskCreate` x N -- one per focus:
   ...
   - Description: `Review <scope> for <focus>. Output severity-rated findings only. Format: [CRITICAL|IMPORTANT|MODERATE] <finding> -- <evidence> -- <recommendation>. No "seems" or "probably" -- concrete evidence only. Save to: {DEVKIT_REPORTS_PATH}/reviewer-{N}-{DEVKIT_NAME_PATTERN}-{scope-slug}.md. Mark task completed when done.`
...
6. **SYNTHESIZE** into: `{DEVKIT_REPORTS_PATH}/review-{scope-slug}.md`
...
## ON `/devkit:agent-team debug <issue>` [--debuggers N]:

*Wraps /devkit:fix skill -- root-cause-first, adversarial hypotheses, disprove to converge.*
...
7. **WRITE** root cause report: `{DEVKIT_REPORTS_PATH}/debug-{issue-slug}.md`
