...
> **Flutter Web apps only:** `enableFlutterAccessibility()` calls are required for Flutter Web (CanvasKit). For non-Flutter apps, omit all `enableFlutterAccessibility()` calls and the Flutter Web notes in comments.
...
## Navigation Pattern: goto() vs navigateTo()

Every POM must implement the correct navigation method based on whether the page is an **entry point** or **navigation-only**.

### Entry Point Page (has direct URL)

```typescript
// ✅ goto() uses URL — only for entry points
async goto(): Promise<void> {
  await this.page.goto('/login');
  await this.page.waitForLoadState('networkidle');
  await this.enableFlutterAccessibility(); // Flutter Web
}
```

**Entry points:** `LoginWithMicrosoftPage`, `HomePage` — pages users can reach directly by typing a URL.

### Navigation-Only Page (reached via nav bar / tabs)

```typescript
// ✅ navigateTo() chains through user navigation — for all other pages
async navigateTo(): Promise<void> {
  const homePage = new HomePage(this.page);
  await homePage.goto();                           // start at entry point
  await homePage.notificationsTabButton.click();   // user clicks nav
  await this.notificationsHeading.waitFor({ state: 'visible' });
  await this.enableFlutterAccessibility();         // re-enable after nav
}

// goto() for navigation-only pages throws or redirects to navigateTo()
async goto(): Promise<void> {
  // For navigation-only pages: use navigateTo() instead
  await this.navigateTo();
}
```

**Navigation-only pages:** `NotificationPage`, `SettingsPage`, `AttendancePage`, `ApprovalRequestsPage`, etc.

**Rule:** If a page is reached by clicking a bottom nav button, tab, or app card — it is navigation-only. Do NOT use `page.goto(url)` for these pages.
...
### File Name
- **Format**: kebab-case, strip trailing `Page` suffix
- **Example**: `LoginPage` → `login.page.ts`, `AttendancePage` → `attendance.page.ts`, `LoginWithMicrosoft` → `login-with-microsoft.page.ts`
- **Rule**: Strip `Page` suffix if present → PascalCase to kebab-case → append `.page.ts`
