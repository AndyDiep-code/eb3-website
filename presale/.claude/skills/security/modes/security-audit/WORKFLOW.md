...
# devkit:security-audit — Security Audit

Runs a structured STRIDE + OWASP security audit on a given scope. Produces a severity-ranked findings report. With `--fix`, applies fixes iteratively using the devkit:autoresearch guard pattern.
...
|------|-----------|----------|
| Audit only | `/devkit:security <scope>` | Scan → categorize → report (one-shot) |
| Red-team discovery | `/devkit:security <scope> --red-team` | Iterate 4 attacker personas → STRIDE/OWASP sweep → report |
| Bounded red-team | `/devkit:security <scope> --red-team --iterations N` | Cap persona discovery to N iterations total |
| Audit + Fix | `/devkit:security --mode security-audit <scope> --fix` | Scan → categorize → fix iteratively |
| Red-team + Fix | `/devkit:security <scope> --red-team --fix` | Full persona discovery → fix confirmed Critical/High |
| Bounded fix | `/devkit:security --mode security-audit <scope> --fix --iterations N` | Limit fix iterations to N |
...
4. Uses `devkit:autoresearch` guard pattern for regression prevention
...
- Run after `devkit:predict` when the security persona flags concerns
- Feed Critical/High findings into `devkit:autoresearch --fix` for automated remediation
- Use `devkit:test-scenario` with `--focus authorization` for deeper auth flow testing
- Pair with `devkit:plan` to schedule Medium/Low findings as sprint tasks
...
/devkit:security --mode security-audit src/api/**/*.ts

# Red-team discovery — full codebase, all 4 personas
/devkit:security --mode security-audit full --red-team

# Red-team discovery — bounded to 20 iterations total
/devkit:security --mode security-audit src/ --red-team --iterations 20

# Red-team discovery + auto-fix confirmed Critical/High
/devkit:security --mode security-audit full --red-team --fix

# One-shot audit + auto-fix, max 15 iterations
/devkit:security --mode security-audit src/ --fix --iterations 15
```
...
See `/devkit:autoresearch` for the full family map.
