...
<!-- Feature Badges -->
<p>
  <a href="https://github.com/7onez/cyber-threat-expert"><img src="https://img.shields.io/badge/version-2.3-0080ff?style=for-the-badge&logo=semver&logoColor=white" alt="Version 2.3"></a>&nbsp;
...
<!-- GitHub Stats -->
<p>
  <a href="https://github.com/7onez/cyber-threat-expert/stargazers"><img src="https://img.shields.io/github/stars/7onez/cyber-threat-expert?style=flat-square&logo=github&label=Stars" alt="Stars"></a>&nbsp;
  <a href="https://github.com/7onez/cyber-threat-expert/network/members"><img src="https://img.shields.io/github/forks/7onez/cyber-threat-expert?style=flat-square&logo=github&label=Forks" alt="Forks"></a>&nbsp;
  <a href="https://github.com/7onez/cyber-threat-expert/releases"><img src="https://img.shields.io/github/downloads/7onez/cyber-threat-expert/total?style=flat-square&logo=github&label=Downloads&color=brightgreen" alt="Downloads"></a>&nbsp;
  <a href="https://github.com/7onez/cyber-threat-expert/issues"><img src="https://img.shields.io/github/issues/7onez/cyber-threat-expert?style=flat-square&logo=github&label=Issues" alt="Issues"></a>&nbsp;
  <a href="https://github.com/7onez/cyber-threat-expert/pulls"><img src="https://img.shields.io/github/issues-pr/7onez/cyber-threat-expert?style=flat-square&logo=github&label=PRs" alt="Pull Requests"></a>&nbsp;
  <a href="https://github.com/7onez/cyber-threat-expert/commits"><img src="https://img.shields.io/github/last-commit/7onez/cyber-threat-expert?style=flat-square&logo=github&label=Last%20Commit" alt="Last Commit"></a>&nbsp;
  <a href="https://github.com/7onez/cyber-threat-expert"><img src="https://img.shields.io/github/repo-size/7onez/cyber-threat-expert?style=flat-square&logo=github&label=Size" alt="Repo Size"></a>&nbsp;
  <a href="https://github.com/7onez/cyber-threat-expert/graphs/contributors"><img src="https://img.shields.io/github/contributors/7onez/cyber-threat-expert?style=flat-square&logo=github&label=Contributors" alt="Contributors"></a>
</p>

<!-- Language Selector -->
<p>
  <a href="#what-is-cyber-threat-expert"><img src="https://img.shields.io/badge/lang-English-blue?style=flat-square" alt="English"></a>&nbsp;
...
|----------|-------------|--------------|
| **Intelligence** | `/cyber-threat-expert /render threat-path`, `/cyber-threat-expert /render attack-surface` | Attack path flow + infrastructure exposure visualization |
| **Intelligence** | `/cyber-threat-expert /snapshots`, `/cyber-threat-expert /diff` | Wayback Machine snapshots and version diffing |
| **Intelligence** | `/cyber-threat-expert /drift`, `/cyber-threat-expert /report ioc` | Temporal risk tracking + IOC export (STIX 2.1) |
| **UX** | `/cyber-threat-expert /onboard`, `/cyber-threat-expert /clarify`, `/cyber-threat-expert /quality` | First-time tutorial, finding explanation, quality scoring |
| **UX** | `/cyber-threat-expert /blind-spots`, `/cyber-threat-expert /source-check` | Gap analysis + batch URL verification |
| **UX** | `/cyber-threat-expert /workspace diff` | Compare two saved investigation sessions |
...
- **Persistent sessions** — investigations survive terminal restarts via `/cyber-threat-expert /workspace save`
- **Full tool access** — file writes, Python scripts, DOCX generation, all run natively
- **Skill invocation** — type `/cyber-threat-expert` directly in the terminal, no browser required
...
```bash
git clone https://github.com/7onez/cyber-threat-expert.git ~/.claude/skills/cyber-threat-expert
bash ~/.claude/skills/cyber-threat-expert/scripts/install.sh
```
...
```powershell
git clone https://github.com/7onez/cyber-threat-expert.git "$env:USERPROFILE\.claude\skills\cyber-threat-expert"
pip3 install -r "$env:USERPROFILE\.claude\skills\cyber-threat-expert\scripts\requirements.txt"
```
...
> If the skill loads, you'll see the CTI Expert command menu. Type `/cyber-threat-expert /help` for the full command list.
...
2. **Download CTI Expert** &mdash; Go to the [GitHub repository](https://github.com/7onez/cyber-threat-expert), click the green **"Code"** button, then select **"Download ZIP"**
3. **Extract to your skills folder** &mdash; Unzip the downloaded file, then move the extracted folder to your skills directory and rename it to `cyber-threat-expert`:
...
6. **Verify** &mdash; Type `/cyber-threat-expert` in the chat to confirm the skill is loaded
...
> **How to run commands:** All commands below use the `/cyber-threat-expert` prefix. Type `/cyber-threat-expert` followed by the command in Claude Code.
>
> Example: `/cyber-threat-expert /case example.com` — not just `/case example.com`

### 1 &mdash; Full Autonomous Case

```bash
/cyber-threat-expert /case example.com
```
...
```bash
/cyber-threat-expert /flow person           # Person investigation workflow
/cyber-threat-expert /flow domain           # Domain reconnaissance workflow
/cyber-threat-expert /flow image            # Image verification workflow
```

### 3 &mdash; Targeted Reconnaissance

```bash
/cyber-threat-expert /sweep @username                    # Multi-vector recon on handle
/cyber-threat-expert /query example.com                  # 12-15 advanced search queries
/cyber-threat-expert /username johndoe                   # Platform enumeration (3000+)
/cyber-threat-expert /email-deep user@domain.com         # Deep email investigation
/cyber-threat-expert /subdomain example.com              # Certificate transparency + brute-force
/cyber-threat-expert /threat-check 185.1.1.1             # IP/domain/URL threat intelligence
/cyber-threat-expert /scam-check suspicious-site.xyz     # Phishing/scam domain check
/cyber-threat-expert /breach-deep user@domain.com        # Multi-source breach lookup
```

### 4 &mdash; Analysis & Assessment

```bash
/cyber-threat-expert /exposure domain.com                # Composite risk score (0-100)
/cyber-threat-expert /threat-model                       # Build threat model from findings
/cyber-threat-expert /validate                           # Verify all findings
/cyber-threat-expert /coverage                           # Check investigation completeness
```

### 5 &mdash; Reporting

```bash
/cyber-threat-expert /report                             # Technical INTSUM report
/cyber-threat-expert /report brief                       # Executive summary
/cyber-threat-expert /brief                              # Plain-language summary
/cyber-threat-expert /workspace save                     # Save workspace + auto-generate .docx
```
...
| Command | Purpose |
|---------|---------|
| `/cyber-threat-expert /case [target]` | Full pipeline — every applicable technique |
| `/cyber-threat-expert /sweep [target]` | Multi-vector recon (person/domain/org/username/email/IP) |
| `/cyber-threat-expert /query [subject]` | 12-15 advanced search operator queries |
| `/cyber-threat-expert /username [handle]` | 3000+ platform enumeration |
| `/cyber-threat-expert /phone [number]` | Carrier lookup, reputation, associations |
| `/cyber-threat-expert /email-deep [email]` | Accounts, breaches, infrastructure |
| `/cyber-threat-expert /subdomain [domain]` | CT logs + passive enumeration |
| `/cyber-threat-expert /threat-check [target]` | IP/domain/URL/hash threat intelligence |
| `/cyber-threat-expert /breach-deep [email]` | Multi-source breach lookup |
...
| Command | Purpose |
|---------|---------|
| `/cyber-threat-expert /branch [data]` | Lateral expansion (email&rarr;username, username&rarr;email, etc.) |
| `/cyber-threat-expert /crossref` | Shared identifier detection across subjects |
| `/cyber-threat-expert /link-subjects [A] [B]` | Define connection between subjects |
| `/cyber-threat-expert /show-connections` | Display logged connections |
| `/cyber-threat-expert /graph` | Full ASCII subject relationship map |
...
| Command | Purpose |
|---------|---------|
| `/cyber-threat-expert /exposure [target]` | Composite risk score (0-100) |
| `/cyber-threat-expert /threat-model` | Build threat model from findings |
| `/cyber-threat-expert /validate` | Verify finding evidence chains |
| `/cyber-threat-expert /coverage` | Check investigation completeness |
...
**Entry:** `/cyber-threat-expert /flow person`, `/cyber-threat-expert /flow domain`, `/cyber-threat-expert /template list`
...
**Entry:** `/cyber-threat-expert /query [target]`, `/cyber-threat-expert /branch [data]`, `/cyber-threat-expert /crossref`, `/cyber-threat-expert /threat-model`
...
**Entry:** `/cyber-threat-expert /username [handle]`, `/cyber-threat-expert /email-deep [email]`, `/cyber-threat-expert /secrets [target]`, `/cyber-threat-expert /threat-check [target]`
...
> Activate with `/cyber-threat-expert /flow [type]` for interactive guided prompts.
...
**Mới trong v2.1:** Trực quan hóa đường tấn công (`/cyber-threat-expert /render threat-path`), bề mặt tấn công (`/cyber-threat-expert /render attack-surface`), xuất IOC STIX 2.1 (`/cyber-threat-expert /report ioc`), theo dõi rủi ro theo thời gian (`/cyber-threat-expert /drift`), ảnh chụp Wayback (`/cyber-threat-expert /snapshots`, `/cyber-threat-expert /diff`), hướng dẫn người mới (`/cyber-threat-expert /onboard`), giải thích phát hiện (`/cyber-threat-expert /clarify`), phân tích khoảng trống (`/cyber-threat-expert /blind-spots`), kiểm tra nguồn (`/cyber-threat-expert /source-check`), so sánh phiên (`/cyber-threat-expert /workspace diff`), điểm chất lượng (`/cyber-threat-expert /quality`), thang độ tin cậy nguồn A-F, 4 loại thực thể mới.
...
- **Phiên làm việc liên tục** — điều tra được lưu qua `/cyber-threat-expert /workspace save`
- **Truy cập đầy đủ công cụ** — ghi file, chạy Python, tạo DOCX, tất cả chạy tự nhiên
- **Gọi skill trực tiếp** — gõ `/cyber-threat-expert` ngay trong terminal
...
2. **Tải CTI Expert** &mdash; Vào [kho GitHub](https://github.com/7onez/cyber-threat-expert), nhấn nút **"Code"** màu xanh, sau đó chọn **"Download ZIP"**
3. **Giải nén vào thư mục skills** &mdash; Giải nén file đã tải, di chuyển thư mục vào thư mục skills và đổi tên thành `cyber-threat-expert`:
...
6. **Xác nhận** &mdash; Gõ `/cyber-threat-expert` trong chat để xác nhận skill đã được tải
...
```bash
/cyber-threat-expert /case example.com                   # Chạy case tự động hoàn toàn
/cyber-threat-expert /flow person                        # Quy trình điều tra cá nhân
/cyber-threat-expert /flow domain                        # Quy trình trinh sát tên miền
/cyber-threat-expert /sweep @username                    # Trinh sát đa vector trên handle
/cyber-threat-expert /query example.com                  # 12-15 truy vấn tìm kiếm nâng cao
/cyber-threat-expert /username johndoe                   # Liệt kê nền tảng (3000+)
/cyber-threat-expert /email-deep user@domain.com         # Điều tra email chuyên sâu
/cyber-threat-expert /exposure domain.com                # Điểm rủi ro tổng hợp (0-100)
/cyber-threat-expert /report                             # Báo cáo kỹ thuật INTSUM
/cyber-threat-expert /workspace save                     # Lưu workspace + tự động tạo .docx
```
...
**v2.1 新功能：** 攻击路径可视化（`/cyber-threat-expert /render threat-path`）、攻击面映射（`/cyber-threat-expert /render attack-surface`）、STIX 2.1 IOC 导出（`/cyber-threat-expert /report ioc`）、时间风险追踪（`/cyber-threat-expert /drift`）、Wayback 快照（`/cyber-threat-expert /snapshots`、`/cyber-threat-expert /diff`）、新手引导（`/cyber-threat-expert /onboard`）、发现解释（`/cyber-threat-expert /clarify`）、盲点分析（`/cyber-threat-expert /blind-spots`）、来源检查（`/cyber-threat-expert /source-check`）、会话比较（`/cyber-threat-expert /workspace diff`）、质量评分（`/cyber-threat-expert /quality`）、来源可靠性 A-F 等级、4 种新实体类型。
...
- **持久会话** — 调查通过 `/cyber-threat-expert /workspace save` 跨重启保存
- **完整工具访问** — 文件写入、Python 脚本、DOCX 生成均原生运行
- **直接调用技能** — 在终端中直接输入 `/cyber-threat-expert`
...
2. **下载 CTI Expert** &mdash; 访问 [GitHub 仓库](https://github.com/7onez/cyber-threat-expert)，点击绿色 **"Code"** 按钮，然后选择 **"Download ZIP"**
3. **解压到 skills 文件夹** &mdash; 解压文件，将文件夹重命名为 `cyber-threat-expert` 并移动到：
...
6. **验证** &mdash; 输入 `/cyber-threat-expert` 确认技能已加载
...
```bash
/cyber-threat-expert /case example.com                   # 完全自动案例
/cyber-threat-expert /flow person                        # 人员调查流程
/cyber-threat-expert /flow domain                        # 域名侦察流程
/cyber-threat-expert /sweep @username                    # 对账号进行多向量侦察
/cyber-threat-expert /query example.com                  # 12-15 个高级搜索查询
/cyber-threat-expert /username johndoe                   # 平台枚举（3000+）
/cyber-threat-expert /email-deep user@domain.com         # 深度电子邮件调查
/cyber-threat-expert /exposure domain.com                # 综合风险评分（0-100）
/cyber-threat-expert /report                             # 技术 INTSUM 报告
/cyber-threat-expert /workspace save                     # 保存工作空间 + 自动生成 .docx
```
