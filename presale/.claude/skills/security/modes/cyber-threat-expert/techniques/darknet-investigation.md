...
Transparent ransomware group claim tracker. Monitors leak sites for new victim posts and exposes data via a public API. Supplements ransomware.live (already in cyber-threat-expert).
...
Step 3: Ransomware victim verification
  ...
  └─ ransomware.live (existing cyber-threat-expert tool): cross-reference
...
### ransomware.live Cross-Reference (existing cyber-threat-expert tool)
```bash
