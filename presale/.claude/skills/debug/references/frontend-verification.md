...
Visual verification of frontend implementations using `devkit:agent-browser`, `devkit:chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests.
...
- No real login/cookies needed: use `devkit:agent-browser`, Chrome MCP, or project-native browser tests.
- Real user login/cookies/profile needed: invoke `devkit:chrome-profile` and run:
...
**ok=false** → Follow the skill's setup playbook, or use `devkit:agent-browser` / project-native browser tests if profile state is not required.
...
When Chrome MCP is not configured, use `devkit:agent-browser`:
