...
Visual verification of frontend implementations via `devkit:agent-browser`, `devkit:chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests. Detect if frontend-related → check browser tool availability → screenshot + console error check → report. Skip if not frontend.
...
- **Frontend:** `devkit:agent-browser`, `devkit:chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests for visual verification
