...
2. Activate `devkit:debug` skill (systematic-debugging + root-cause-tracing techniques).
...
- `devkit:brainstorm` — multiple valid approaches, architecture decision (Deep only)
...
**Typically follows:** `/devkit:debug` (after root cause analysis), `/scout` (after locating affected code)
**Typically precedes:** `/devkit:code-review` (review the fix), `/devkit:test` (validate the fix)
**Related:** `/devkit:cook` (alternative for feature work), `/devkit:debug` (diagnose before fixing)
