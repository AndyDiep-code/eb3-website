<!--
Usage: /devkit:review --mode plan-security [task-id | plan-path]
Example: /devkit:review --mode plan-security ABC-123
Example: /devkit:review --mode plan-security ./.claude/tasks/ABC-123/plan/
-->
...
1. If args provided:
   - If contains `/` → Use as plan path directly
   - Else → Resolve as task ID: `./.claude/tasks/$ARGUMENTS/plan/`
2. Else check `$DEVKIT_TASK_ID` env var (set by session hooks) → Resolve to `./.claude/tasks/{task-id}/plan/`
3. Else check `## Plan Context` section → Use active plan path
4. If no plan found → Ask user to specify task ID or path, or run `/devkit:plan` first

Run `node .claude/scripts/set-active-task.cjs {task-id}` if task-id was resolved and not already active.
...
Present adjudicated findings via `AskUserQuestion`:
...
Record the modified suggestion. Set disposition to "Accept (modified)".
...
> ```
> /devkit:review --mode plan {task-id}
> ```
> to re-validate decisions after changes, then:
> ```
> /devkit:implement {task-id}
> ```
> to implement.
...
**IMPORTANT:** If plan has a Validation Log from plan validation, reviewers should check if validation answers introduced new assumptions.
