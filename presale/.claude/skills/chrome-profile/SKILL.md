...
This is not the default for ordinary browser testing. Use `devkit:agent-browser` when a fresh or tool-managed browser is enough. Use project-native Playwright or `devkit:web-testing` for repeatable CI tests. Use this skill when the user's actual Chrome profile state matters.
...
```bash
devkit:chrome-profile work "https://github.com/org/repo/pulls"
```
...
Do not use an MCP `new_page` tool for non-default profiles. It opens in whichever profile the bridge defaults to. Always materialize profile-specific tabs with `devkit:chrome-profile <key> <url>`.
...
| Missing profile key | Run `devkit:chrome-profile setup` or edit the per-machine config. |
| Unresolved key | The Google account is not signed into this Chrome profile on this machine. Sign in once through Chrome UI. |
| No bridge | Run `devkit:chrome-profile doctor` and follow Option A or B above. |
