...
```
/devkit:cook <natural language task OR plan path>
```
...
- `--auto`: Auto-approve low-risk steps; high-risk changes stop for human approval before finalize
...
**Example:**
```
/devkit:cook "Add user authentication to the app" --fast
/devkit:cook path/to/plan.md --auto
/devkit:cook "Refactor auth middleware" --tdd
```
<HARD-GATE>
...
| Finalize | `/project-management` skill + `docs-manager` subagents | **MUST** invoke all |
...
**Typically follows:** `/devkit:plan` (execute a plan), `/devkit:brainstorm` (implement agreed solution)
**Typically precedes:** `/devkit:code-review` (review after implementation), `/devkit:test` (validate changes)
**Related:** `/devkit:fix` (alternative for bug fixes), `/devkit:plan` (create plan before cooking)
