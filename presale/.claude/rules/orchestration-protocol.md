...
2. **Reports Path**: `$DEVKIT_REPORTS_PATH` (injected by hooks, resolves to `.claude/tasks/[task-id]/reports/`)
3. **Plans Path**: `$DEVKIT_ACTIVE_PLAN` (injected by hooks, resolves to `.claude/tasks/[task-id]/plan/`)

Hooks inject these paths automatically via `subagent-init.cjs` — subagents receive `## Naming` and `## Context` sections with pre-computed paths. No need to hardcode.

To set active task: `node .claude/scripts/set-active-task.cjs [task-id]`
...
For multi-session parallel collaboration, activate the `/devkit:agent-team` skill.
Not part of the default orchestration workflow. See `.claude/skills/agent-team/SKILL.md` for templates, decision criteria, and spawn instructions.
