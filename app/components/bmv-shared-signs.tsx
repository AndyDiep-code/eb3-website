// Shared traffic signs reference — federal-standard content used by all state BMV pages.

export function BmvSharedSigns() {
  return (
    <div>
      <div className="mb-3 border-b border-border pb-2 text-base font-bold text-text">
        🚦 Biển Báo Giao Thông Hoa Kỳ
      </div>
      ...
      <div className="mb-4 flex gap-3 rounded-card border border-primary/20 bg-primary/5 p-3 text-sm text-text">
        ...
        <div>
          ...
          — học màu trước, học hình sau, rồi học nội dung. Đây là một trong những
          phần hay ra câu hỏi nhất trong bài thi.
        </div>
      </div>
          ...
          <tbody>
            {[
              ...
              <tr key={i} className={i % 2 === 0 ? "bg-bg" : "bg-bg-alt"}>
                ...
                <td className="px-3 py-2 leading-relaxed text-text">{row.meaning}</td>
                <td className="px-3 py-2 leading-relaxed text-text-muted">{row.example}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table className="w-full border-collapse text-xs">
          ...
          <tbody>
            {[
              ...
              <tr key={i} className={i % 2 === 0 ? "bg-bg" : "bg-bg-alt"}>
                <td className="px-3 py-2 font-medium leading-relaxed text-text">{shape}</td>
                <td className="px-3 py-2 leading-relaxed text-text-muted">{meaning}</td>
                <td className="px-3 py-2 leading-relaxed text-text-muted">{example}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table className="w-full border-collapse text-xs">
          ...
          <tbody>
            {[
              ...
              ["SCHOOL CROSSING",     "Lối đi bộ gần trường",  "Giảm tốc còn 20–25 mph khi có học sinh. Nhường học sinh luôn luôn."],
              ...
              <tr key={i} className={i % 2 === 0 ? "bg-bg" : "bg-bg-alt"}>
                ...
                <td className="px-3 py-2 italic text-text-muted">{vi}</td>
                <td className="px-3 py-2 leading-relaxed text-text-muted">{action}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-4 rounded-card border border-border bg-bg p-4">
        <div className="mb-1 text-sm font-bold text-text">🖼️ Thư Viện Biển Báo Hình Ảnh</div>
...
function SignSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-5">
      <div className="mb-2 border-b border-border pb-1 text-sm font-semibold text-primary">
        {title}
      </div>
      <div className="overflow-x-auto rounded-card border border-border">{children}</div>
    </div>
  );
}
