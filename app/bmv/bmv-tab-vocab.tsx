...
// Tab 2: Từ Vựng — Vocabulary tables with topic filter. Ported from bmv.html #vocab.
...
const V1: VocabRow[] = [
  ...
  { en: "Seatbelt / Safety belt", vi: "Dây an toàn", note: "Indiana law: all passengers must wear seatbelts.", hot: true },
...
const V2: VocabRow[] = [
  ...
  { en: "School zone sign", vi: "Biển báo khu vực trường học", note: "Speed limit = 20 mph when children are present.", hot: true },
...
const V3: VocabRow[] = [
  { en: "Speed limit", vi: "Giới hạn tốc độ", note: "Rural interstate: 70 mph. Urban interstate: 55 mph.", hot: true },
  { en: "DUI — Driving Under the Influence", vi: "Lái xe khi say rượu/ma túy", note: "BAC ≥ 0.08% = DUI. Penalty: arrest, license suspension, jail.", hot: true },
  { en: "Blood Alcohol Content (BAC)", vi: "Nồng độ cồn trong máu", note: "Legal limit: 0.08%. Under 21: 0.02%." },
  { en: "Reckless driving", vi: "Lái xe ẩu / liều lĩnh", note: "Can result in license suspension or criminal charges." },
  { en: "Hit and run", vi: "Gây tai nạn rồi bỏ chạy", note: "Felony in Indiana. Must stop and exchange information." },
  ...
  { en: "Distracted driving", vi: "Lái xe mất tập trung (nhắn tin, gọi điện...)", note: "Indiana bans texting while driving. $500+ fine.", hot: true },
];
...
const V5: VocabRow[] = [
  ...
  { en: "Learner's permit", vi: "Giấy phép tập lái xe", note: "First step. Valid 2 years. Must drive with licensed adult 25+." },
  { en: "Knowledge test / Written test", vi: "Bài thi lý thuyết", note: "50 questions. Available in Vietnamese at BMV!" },
  { en: "Road test / Driving skills test", vi: "Bài thi lái xe thực hành", note: "With a BMV examiner. ~20 minutes. Fee ~$18." },
  ...
  { en: "Proof of residency", vi: "Bằng chứng địa chỉ cư trú", note: "Utility bill, bank statement with Indiana address." },
  { en: "Green card / Permanent Resident Card", vi: "Thẻ xanh / Thẻ thường trú nhân", note: "Bring to BMV as proof of legal status." },
...
const TOPIC_DATA: Record<TopicKey, VocabRow[]> = {
  v1: V1,
  v2: V2,
  v3: V3,
  v4: V4,
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr
                key={i}
                className={i % 2 === 0 ? "bg-bg" : "bg-bg-alt"}
              >
                <td className="px-3 py-2 font-medium text-text align-top leading-relaxed">
                  {row.en}
                </td>
                <td className="px-3 py-2 text-text-muted italic align-top leading-relaxed">
                  {row.vi}
                </td>
                <td className="px-3 py-2 text-text-muted align-top leading-relaxed">
                  {row.note}
                  {row.hot && (
                    <span className="ml-1 text-secondary font-bold">⚡ HAY THI</span>
                  )}
                </td>
              </tr>
            ))}
