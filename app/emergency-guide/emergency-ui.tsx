// Shared presentational primitives for the emergency-guide tab content
// (heading, sub-label, card, alert, do/don't box, phrase block) — extracted
// so each tab file (emergency-tab-*.tsx) only contains content, not
// repeated markup. Ported from emergency-guide.html's .card/.alert/
// .phrase-block/.do-box/.dont-box CSS classes.
...
type AlertTone = "green" | "red" | "blue" | "yellow" | "orange";
...
const ALERT_TONE_CLASSES: Record<AlertTone, string> = {
  ...
  orange: "border-orange-600/40 bg-orange-600/10 text-text",
};
...
export function PhraseBlock({
  ...
  return (
    <div className="mb-2 rounded-card border border-primary/40 bg-primary/10 p-3">
      <div className="text-sm font-bold italic text-primary">{en}</div>
      <div className="mt-1 text-xs text-text-muted">{vi}</div>
...
export function StepRow({
  ...
  return (
    <div className="mb-2.5 flex items-start gap-2.5">
      <div
        className={`mt-0.5 flex h-6.5 w-6.5 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold text-white ${numColor}`}
      >
        {num}
      </div>
