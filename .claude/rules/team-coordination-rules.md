...
### Report Output
- Save reports to `$DEVKIT_REPORTS_PATH` (injected via hooks, resolves to `.claude/tasks/[task-id]/reports/`)
- Use the `## Naming` section injected by `subagent-init.cjs` — includes full path with date and agent type
