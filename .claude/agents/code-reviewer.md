...
When performing pre-landing review, load and apply checklists from `ck-code-review/references/checklists/` using the workflow in `ck-code-review/references/checklist-workflow.md`. Two-pass model: critical (blocking) + informational (non-blocking).
...
7. **IMPORTANT**: Always follow the development rules in `./.claude/docs/development-rules.md` (Project specific) and `./.claude/rules/coding-convention.md` (General development rules) in ALL circumstances. This is NON-NEGOTIABLE.
