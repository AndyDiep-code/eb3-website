...
4. Existing docs in `./docs/` and any related plans in `.claude/task/plans/`
...
flowchart TD
    ...
    H --> I{Create Plan?}
    I -->|Yes| J[Pick /devkit:plan mode<br/>--tdd or default]
    I -->|No| K[End Session]
    J --> L[Journal]
    K --> L
```

**This diagram is the authoritative workflow.** If prose conflicts with this flow, follow the diagram. The terminal state is either `/devkit:plan` or end.
...
8. **Finalize Phase (Plan Handoff)**: Once the user has confirmed the proposal AND has no further questions (i.e. brainstorm is converging to close), use `AskUserQuestion` to offer the appropriate `/devkit:plan` mode. Pass the brainstorm summary path as context to `/devkit:plan` for continuity.
   ...
   **Plan mode selection — present these as options:**
   ...
   |--------|----------------|-----|
   | `/devkit:plan --tdd` | Solution refactors existing behavior, modifies critical business logic, or has strong existing test coverage to preserve | Forces tests-first per phase so current behavior is locked in before changes |
   | `/devkit:plan` (default) | Standard new feature or moderate change | Produces the standard phase-by-phase implementation plan |
   ...
   **Note:** `/devkit:plan validate` and `/devkit:plan red-team` are post-plan gates — do NOT offer them here. They are surfaced by `/devkit:plan` itself after the plan is produced.
...
**Typically follows:** `/devkit:debug` (brainstorm solutions for diagnosed issues), `/scout` (brainstorm after discovery)
**Typically precedes:** `/devkit:plan` (plan the agreed solution)
**Related:** `/devkit:plan` (plan after brainstorming), `/devkit:debug` (debug before brainstorming)