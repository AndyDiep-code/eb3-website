...
`devkit:fix` and `devkit:cook` use these JSON artifacts to make review and finalize gates deterministic. Scores are advisory. Approval comes from structured proof plus the validator.
...
```json
{
  "artifactDir": ".claude/tasks/example/reports/harness",
  "planPath": ".claude/tasks/example/plans",
  "skill": "devkit:cook",
  "mode": "auto",
