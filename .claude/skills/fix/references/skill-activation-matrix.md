...
| `devkit:debug` | Step 2 | Systematic root cause investigation |
...
| Skill | Activate When |
|-------|---------------|
| `devkit:brainstorm` | Multiple valid fix approaches, architecture decision (Deep only) |
...
|----------|------------------|
| Quick | `scout` (minimal), `devkit:debug`, `sequential-thinking`, `devkit:code-review`, `project-management`, parallel `Bash` verification |
| Standard | Above + Tasks, `problem-solving` (auto), `project-management`, `tester`, parallel `Explore` |
| Deep | All above + `devkit:brainstorm`, `researcher`, `planner` |
...
| Step 2: Diagnose | Capture pre-fix state → `devkit:debug` → `sequential-thinking` → parallel `Explore` hypotheses → (`problem-solving` if 2+ fail) |
...
| Step 6: Finalize | Report → `project-management` (MANDATORY) → `docs-manager` → `TaskUpdate` |
...
| "which approach", "options" | `devkit:brainstorm` |
