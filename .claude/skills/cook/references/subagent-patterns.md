...
## Scout Phase (use /scout command)
```
/scout "Find files related to [feature] in codebase"
```
- Use `/scout "prompt"` for codebase exploration
...
```
- Input: researcher reports and /scout findings
...
```
- Trigger when live `git diff --numstat HEAD --ignore-all-space` breaches any `simplify.threshold` from `.devkit.json` (defaults: 400 LOC / 8 files / 200 single-file LOC)
...
- Skip when `DEVKIT_SIMPLIFY_DISABLED=1` or `.devkit.json` `simplify.gate.enabled=false`
