...
FUNCTION detectMode(input):
  ...
  IF input matches path pattern (.claude/tasks/*/plan/*, plan.md, phase-*.md):
    RETURN "code"
...
```
"/devkit:cook implement user auth --interactive"
→ Mode: interactive (explicit flag, stops at review gates)

"/devkit:cook implement user auth"
→ Mode: interactive (default, stops at review gates)

"/devkit:cook .claude/tasks/AUTH-001/plan/phase-02-api.md"
→ Mode: code (path detected, stops at review gates)

"/devkit:cook quick fix for the login bug"
→ Mode: fast ("quick" keyword, stops at review gates)

"/devkit:cook implement auth, payments, notifications, shipping"
→ Mode: parallel (4 features, stops at review gates)

"/devkit:cook implement dashboard --fast"
...
"/devkit:cook implement everything --auto"
→ Mode: auto (continuous only for low-risk, artifact-validated work)

"/devkit:cook implement dashboard trust me"
