...
- Use `/scout "prompt"` for codebase search
...
- Use `AskUserQuestion` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `/devkit:review --mode plan` skill
...
**IMPORTANT:**
1. `TaskList` first — check for existing tasks (hydrated by plan skill in same session)
...
Read thresholds from `.devkit.json` (`simplify.threshold.{locDelta,fileCount,singleFileLoc}`),
...
Skip the step entirely when `DEVKIT_SIMPLIFY_DISABLED=1` or
`.devkit.json` `simplify.gate.enabled` is `false`.
...
**CRITICAL:** Step 6 is INCOMPLETE without activating `/project-management` skill AND spawning `docs-manager` subagents. DO NOT skip.

**Auto mode:** Continue to next phase automatically, start from **Step 3**.
**Others:** Use `AskUserQuestion` to ask user before next phase
...
- **MANDATORY DELEGATION:** Steps 4, 5, 6 MUST delegate via Task tool / skill activation. DO NOT implement directly.
  ...
  - Step 6: `/project-management` skill, `docs-manager`
