...
stage: plan  # Workflow column: ready → requirement → plan → implement → test → done
openapi-spec: path/to/openapi.yaml  # if schema-first (omit if not applicable)
generator-config: path/to/config    # if schema-first (omit if not applicable)
---
```
...
PLAN FILES:
  ./.claude/tasks/[task-id]/plan/plan.md
  ./.claude/tasks/[task-id]/plan/phase-01-*.md
  ./.claude/tasks/[task-id]/plan/phase-02-*.md
  ...
  ./.claude/tasks/[task-id]/plan/phase-XX-code-review.md

PHASES: [count] phases planned
REUSABLE RESOURCES: [count] existing code/patterns identified

NEXT STEPS:
  1. Review plan in ./.claude/tasks/[task-id]/plan/
  2. Run /devkit:implement [task-id] to start implementation
