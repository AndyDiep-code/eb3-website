...
4. **Implementation option:** `/devkit:cook {absolute-plan-path}/plan.md`
...
7. **Context reminder:** `/devkit:cook {absolute-plan-path}/plan.md`
...
8. Output the standard `/devkit:cook {absolute-plan-path}/plan.md` reminder
...
7. **Context reminder:** `/devkit:cook --parallel {absolute-plan-path}/plan.md`
...
**Invocation:** Run `/devkit:plan red-team {plan-directory-path}`.
```
/devkit:plan red-team {plan-directory-path}
```
...
| `auto` | Run `/devkit:plan validate {plan-directory-path}` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Run:
```
/devkit:plan validate {plan-directory-path}
```
...
| Mode | Cook Command |
|------|-----------------------------|
| fast | `/devkit:cook {path}/plan.md` |
| hard | `/devkit:cook {path}/plan.md` |
| deep | `/devkit:cook {path}/plan.md` |
| parallel | `/devkit:cook --parallel {path}/plan.md` |
| two | `/devkit:cook {path}/plan.md` |

If planning ran with `--tdd`, append `--tdd` to the reminder above so cook keeps the tests-first execution path. Example:
`/devkit:cook {path}/plan.md --tdd`
