...
# devkit:predict — Multi-Persona Pre-Analysis
...
- Trivial or low-risk changes (use `devkit:debug` for bugs, `devkit:plan` for already-decided tasks)
...
These chain modes absorb upstream `/autoresearch:reason` and `/autoresearch:probe` ([uditgoenka/autoresearch](https://github.com/uditgoenka/autoresearch), MIT). They're folded into devkit:predict — not shipped as standalone skills — because they always chain off a predict invocation. See `/devkit:autoresearch` for the family map.
...
|---------------|-------|-----|
| Deepen risk scenarios | `devkit:scenario` | Feed Risk Summary rows as feature description |
| Create implementation plan | `devkit:plan` | Attach Recommendations as constraints to planner |
| High-risk feature implementation | `devkit:cook` | Reference CAUTION/STOP items as acceptance gates |
...
```
/devkit:predict "Add WebSocket support for real-time notifications"
/devkit:predict "Migrate authentication from JWT to session cookies"
/devkit:predict "Add multi-tenancy to the database layer"
/devkit:predict "Replace REST API with GraphQL" --files src/api/**/*.ts


# Chain modes
/devkit:predict "Pick auth library: Passport vs Better Auth" --chain reason
/devkit:predict "Move from REST to GraphQL" --chain probe
```
