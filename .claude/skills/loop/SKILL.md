---
name: devkit:loop
description: "Autonomous iterative optimization loop — run N iterations against a mechanical metric, backup/restore files on discard, auto-keep/discard changes. Use for improving measurable metrics (coverage, performance, bundle size, etc.) through repeated experimentation."
user-invocable: true
...
|-----------|-------------|
| Subjective goals ("make it cleaner") | `devkit:cook` |
| Bug fixing with known root cause | `devkit:fix` or `devkit:debug` |
| One-shot tasks, no repetition needed | `devkit:cook` |
| No mechanical metric to measure progress | `devkit:cook --interactive` |
...
- Backup BEFORE verify — save original file contents to `.claude/loop-backup/iter-N/` before running the verify command
- Guard files are **read-only** — never modify files in guard command's scope
- Restore from backup on discard — never delete `.claude/loop-backup/` until the loop is fully complete

## Results Logging

Each iteration appends a TSV line to `.claude/loop-results.tsv` in the working directory:
...
- Requires write access to the working directory (creates `.claude/loop-backup/` and `.claude/loop-results.tsv`)
...
- [`references/git-memory-pattern.md`](references/git-memory-pattern.md) — Loop memory pattern: `.claude/loop-results.tsv` + `.claude/loop-backup/` as cross-iteration memory
...
See `/devkit:autoresearch` for the full family map and what's not yet absorbed.