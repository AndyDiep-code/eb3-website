...
- `/devkit:preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/devkit:preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/devkit:preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/devkit:preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)
- `/devkit:preview --html --explain <topic>` - Self-contained HTML explanation
- `/devkit:preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `/devkit:preview --html --diagram <topic>` - HTML diagram with zoom controls
- `/devkit:preview --html --diff [ref]` - Visual diff review
- `/devkit:preview --html --plan-review [plan-file]` - Plan vs codebase comparison
- `/devkit:preview --html --recap [timeframe]` - Project context snapshot
...
4. **Path doesn't resolve to a generation flag** → Ask user to clarify
...
| Flag without topic | Ask user: "Please provide a topic: `/devkit:preview --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `/devkit:preview --stop` first |
