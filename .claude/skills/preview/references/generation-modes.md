...
For full SVG layout rules (spacing minimums, arrow connection points, z-index ordering, anti-pattern catalog), read `/devkit:tech-graph`'s `references/svg-layout-best-practices.md`. For full publish-grade SVG+PNG diagram generation (7 visual styles, agent/memory primitives), use `/devkit:tech-graph` directly.

## Step 3: Save

Write generated content to determined path.
