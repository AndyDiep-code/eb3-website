...
# devkit:test-scenario — Edge Case & Scenario Explorer
...
| Next Step | Skill | How |
|-----------|-------|-----|
| Generate test cases from scenarios | `devkit:test` | Pass scenario table as input context |
| Inform implementation plan risks | `devkit:plan` | Paste Critical/High rows into risk assessment |
| Deep persona debate on top risks | `devkit:predict` | Feed Critical scenarios as the change proposal |
...
→ `.claude/skills/test-scenario/references/saturation-loop.md`
...
```
/devkit:test-scenario src/api/payment.ts
/devkit:test-scenario "User registration with OAuth providers"

# Bounded iterative — exactly 25 iterations
/devkit:scenario src/api/payment.ts --iterations 25

# Saturation — stop when coverage exhausted
/devkit:scenario "Add multi-tenancy to the database layer" --saturation

# Saturation with domain hint for priority dimension ordering
/devkit:scenario src/middleware/auth.ts --saturation --domain security
```
